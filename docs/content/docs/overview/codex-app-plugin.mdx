...
```text
<画布网页地址>/canvas?mode=new#agentUrl=<URL 编码后的 Local URL>&agentToken=<URL 编码后的 Connect token>
```

`mode=new` 会让网页自动创建具体画布并连接本地 Agent，不需要手动点击“新建画布”。Agent 地址只允许本机回环地址，token 放在 fragment 中，不会发送到静态站点服务器。
