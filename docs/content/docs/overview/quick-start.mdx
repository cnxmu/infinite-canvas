...
在 Vercel 中导入仓库并以 `web/` 为项目根目录，使用 `web/vercel.json` 构建静态前端。当前版本的 AI 请求由浏览器前台直连固定接口站点，不需要额外配置项目服务端。
...
- 先打开右上角配置弹窗，在 `https://img.xmu.la` 和 `https://aiba.hk` 中选择接口站点，并填入自己的 `API Key` 和模型名。
...
- AI API Key 保存在浏览器本地，并由前端直接请求所选固定接口站点。
