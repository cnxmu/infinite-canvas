...
Claude Code 侧不再保留通过系统 shell 启动 CLI 的 Adapter。后续接入直接使用 Claude Agent SDK，由 `canvas-agent` 统一转换事件和工具调用。
...
Claude Code 通过 MCP 操作画布时，真正修改画布仍由网页侧边栏确认。
...
1. Canvas Agent 新增 Claude Agent SDK Adapter。
